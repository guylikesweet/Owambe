...
def init_db():
    ...
    # Event controls. A blank event_date keeps normal ticket sales open until
    # an administrator sets the event date. scan_mode defaults to automatic:
    # scanning becomes available on the event date and ticket sales close.
    query("INSERT INTO app_settings (key, value) VALUES ('event_date', %s) ON CONFLICT (key) DO NOTHING", ("",))
    query("INSERT INTO app_settings (key, value) VALUES ('scan_mode', %s) ON CONFLICT (key) DO NOTHING", ("auto",))

    # Allow removed sellers to be permanently deleted without deleting their
    # ticket/audit history. Existing foreign keys are migrated to SET NULL.
    for table, constraint, column, ref_table in (
        ('tickets', 'tickets_sold_by_fkey', 'sold_by', 'users'),
        ('tickets', 'tickets_cancelled_by_fkey', 'cancelled_by', 'users'),
        ('tickets', 'tickets_refunded_by_fkey', 'refunded_by', 'users'),
        ('ticket_audit', 'ticket_audit_performed_by_fkey', 'performed_by', 'users'),
        ('walkin_tables', 'walkin_tables_created_by_fkey', 'created_by', 'users'),
    ):
        query(f"ALTER TABLE {table} DROP CONSTRAINT IF EXISTS {constraint}")
        query(f"ALTER TABLE {table} ADD CONSTRAINT {constraint} FOREIGN KEY ({column}) REFERENCES {ref_table}(id) ON DELETE SET NULL")
    db.commit()
...
def get_event_date():
    row = query("SELECT value FROM app_settings WHERE key = 'event_date'").fetchone()
...
def get_scan_mode():
    row = query("SELECT value FROM app_settings WHERE key = 'scan_mode'").fetchone()
    mode = (row["value"] if row else "auto").strip().lower()
...
def is_scan_enabled():
    ...
    return bool(event_date and lagos_now().date() >= event_date)

def are_sales_open():
    event_date = get_event_date()
    return not event_date or lagos_now().date() < event_date

def event_date_display(event_date):
    return event_date.strftime("%A, %d %B %Y") if event_date else None
...
def update_event_settings():
    ...
    scan_mode = request.form.get("scan_mode", "auto").strip().lower()
    if scan_mode not in {"auto", "enabled", "disabled"}:
        flash("Invalid scan setting.", "error")
        return redirect(url_for("report"))
    ...
    if raw_date:
        try:
            ...
            return redirect(url_for("report"))
        if selected_date < lagos_now().date():
            flash("Event date cannot be in the past.", "error")
            return redirect(url_for("report"))

    query("INSERT INTO app_settings (key, value) VALUES ('event_date', %s) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value", (raw_date,))
    query("INSERT INTO app_settings (key, value) VALUES ('scan_mode', %s) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value", (scan_mode,))
    get_db().commit()
    if raw_date:
        display = event_date_display(date.fromisoformat(raw_date))
        flash(f"Event date set to {display}. Ticket sales will close automatically on that date.", "success")
    else:
        flash("Event date cleared. Ticket sales remain open until a date is set.", "success")
    return redirect(url_for("report"))
...
def report():
    ...
    return render_template("report.html", sales=sales, users=users, overall=overall, prices=prices, categories=CATEGORY_SLUGS, event_name=EVENT_NAME, user=g.user, commission_rate=commission_rate, walkin_prices=walkin_prices, walkin_categories=WALKIN_CATEGORY_SLUGS, max_tickets=max_tickets, event_date=event_date, scan_mode=scan_mode, scan_enabled=is_scan_enabled(), sales_open=are_sales_open())
...
def home():
    ...
    return render_template("home.html", overall=overall, mine=mine, event_name=EVENT_NAME, user=user, ticket_price=get_ticket_price(), max_tickets=max_tickets, remaining_tickets=remaining_tickets, event_date=get_event_date(), sales_open=are_sales_open())
...
def sell():
    ...
    max_tickets = get_max_tickets()
    event_date = get_event_date()
    sales_open = are_sales_open()
    if not sales_open:
        return render_template("sell.html", overall=query("SELECT COUNT(*) AS sold, COALESCE(SUM(CASE WHEN used THEN 1 ELSE 0 END),0) AS used, COALESCE(SUM(amount_paid - COALESCE(refund_amount,0)),0) AS cash FROM tickets WHERE cancelled = FALSE").fetchone(), mine=query("SELECT COUNT(*) AS sold, COALESCE(SUM(CASE WHEN used THEN 1 ELSE 0 END),0) AS used, COALESCE(SUM(amount_paid - COALESCE(refund_amount,0)),0) AS cash FROM tickets WHERE cancelled = FALSE AND sold_by = %s", (user["id"],)).fetchone(), recent=query("SELECT t.*, u.username FROM tickets t LEFT JOIN users u ON t.sold_by = u.id ORDER BY t.id DESC LIMIT 2").fetchall(), prices=prices, categories=CATEGORY_SEATS, event_name=EVENT_NAME, user=user, max_tickets=max_tickets, remaining_tickets=None, event_date=event_date, sales_open=False)
    if request.method == "POST":
    ...
    return render_template(
        ...
        event_date=event_date, sales_open=sales_open,
    )
...
def scan():
    event_date = get_event_date()
    scan_enabled = is_scan_enabled()
    scan_mode = get_scan_mode()
    if not scan_enabled:
        if scan_mode == "disabled":
            scan_block_reason = "Scanning has been disabled by an administrator."
        elif not event_date:
            scan_block_reason = "The event date has not been set yet, so scanning is locked to prevent accidental check-ins."
        else:
            scan_block_reason = f"Scanning will automatically open on {event_date_display(event_date)}."
    else:
        scan_block_reason = None
    return render_template("scan.html", event_name=EVENT_NAME, user=g.user, event_date=event_date, event_date_display=event_date_display(event_date), scan_enabled=scan_enabled, scan_mode=scan_mode, scan_block_reason=scan_block_reason)
...
def scan_api_blocked():
    if is_scan_enabled():
        return None
    event_date = get_event_date()
    if get_scan_mode() == "disabled":
        message = "Scanning is currently disabled by an administrator."
    elif not event_date:
        message = "Scanning is locked because the event date has not been set."
    else:
        message = f"Scanning is locked until {event_date_display(event_date)}."
    return {"status": "SCAN_DISABLED", "msg": message}
